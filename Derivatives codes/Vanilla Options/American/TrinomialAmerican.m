function [] = TrinomialAmerican(CallPut, AssetP, Strike, RiskFree, Div, Time, Vol, nSteps)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for TT = (nSteps - 1):-1:0
    for i = 0:(TT * 2)
        State = i + 1;
        Value(State) = max((P_up * Value(State + 2) + P_mid * Value(State + 1) + P_down * Value(State)) * Df, b * (AssetP * Up ^ max(i - TT, 0) * Down ^ max(TT *2 - TT - i, 0) - Strike));    
    end
end
